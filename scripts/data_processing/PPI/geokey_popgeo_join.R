#### Joining geokeys with subregionalcovid19 population-geometry data ####
#### Jordan Klein
...
st_write(Geocoded.coords, "../../../data/raw/Geocoded_coords.shp", append = F)
...
## Recode brussels -> level 3 subdivision
To.geocode <- mutate(To.geocode, lvl_num = ifelse(grepl("BRUSSELS CAPITAL", location_to_geocode) & lvl_num == 1, 3, lvl_num))
...
table(Geokeys.tojoin_matchespossible$country.isocode, !is.na(Geokeys.tojoin_matchespossible$geoid))
...
geo = split(Geo.Pop, Geo.Pop$iso3)[!grepl("AUS|DNK", Geolvl_table$iso3)], SIMPLIFY = F) %>% 
    bind_rows()
...
##### Tabulate progress
table(!is.na(Geokeys.tojoin_matchespossible$geoid))
...
geo = split(Geo.Pop, Geo.Pop$iso3)[grepl(paste(unique(Locations_nomatch$country.isocode), collapse = "|"), Geolvl_table$iso3)], 
SIMPLIFY = F) %>% 
    bind_rows()
...
#### Manually fix some cases
# Canary islands
Geokeys.tojoin_matchespossible$geoid[grepl("CANARY ISLANDS", Geokeys.tojoin_matchespossible$geo_l3) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "ESP724_04_35"
# Balearic islands
Geokeys.tojoin_matchespossible$geoid[grepl("BALEARIC ISLAND", Geokeys.tojoin_matchespossible$geo_l3) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "ESP724_03_07"
# Naples
Geokeys.tojoin_matchespossible$geoid[grepl("CAMPANIA", Geokeys.tojoin_matchespossible$geo_l3) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "ITA380_15_063"
# Penang
Geokeys.tojoin_matchespossible$geoid[grepl("PENANG|PINANG", Geokeys.tojoin_matchespossible$geo_l3) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "MYS458_00_10"
# Waterland (Netherlands)
Geokeys.tojoin_matchespossible$geoid[grepl("MARKEN", Geokeys.tojoin_matchespossible$geo_l4) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "NLD528_PV27_GM0852"
# Aransas Co, TX
Geokeys.tojoin_matchespossible$geoid[grepl("FULTON", Geokeys.tojoin_matchespossible$geo_l4) & 
                                         is.na(Geokeys.tojoin_matchespossible$geoid)] <- "USA840_TX_48007"

## rest of NAs = garbade codes 
#(either mismatch b/w country & subnational division, state/county, or location provides no info to identify subnational division)
...
Geokeys_IDed <- left_join(Geo_keys, 
                          select(Geokeys.tojoin_matchespossible, loc_id, geoid))

#### Export
write_csv(Geokeys_IDed, '../../../data/raw/geo_keys_ids.csv')
