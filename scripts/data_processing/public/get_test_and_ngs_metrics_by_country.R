...
stopifnot('More than 40 countries are missing average daily TPR from FIND' = nrow(no_avg_tpr)<=40)
...
stopifnot('Less than 75 LMICs with avg daily TPR not NA & with tests reported in last 6 months' = 
            nrow(find_clean_LMICs%>%
                   filter(!is.na(avg_tpr_find))%>%filter(rept_tests_within_last_6_months == TRUE)) >= 75)
